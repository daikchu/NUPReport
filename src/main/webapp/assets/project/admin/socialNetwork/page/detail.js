 ...
 */
app.controller('socialFanpageDetailCtrl', ['$scope', '$http', '$filter', '$window', '$timeout', '$q'
    , function ($scope, $http, $filter, $window, $timeout, $q) {
        ...
        var paramGetUserInfoByPageId = "{\n" +
            "  \"ids\": [\n" +
            "    \"" + $scope.pageId + "\"\n" +
            "  ]\n" +
            "}";

        $http.post(preUrl + "/social/pages/getPageInfo", paramGetUserInfoByPageId, {headers: {'Content-type': 'application/json; charset=utf-8;'}})
            .then(function (response) {
                if (response.data !== "") {
                    $scope.pageInfo = response.data.data.docs[0];
...
//HÀM XỬ LÝ THƯ VIỆN NHẬP KHOẢNG THỜI GIAN
        ...
        $scope.viewChartKeywordByType = function(){
            switch ($scope.typeChartKeyword) {
                case 'post':chartKeyword($scope.dataStatistic.keywordPostCount.buckets);
                    break;
                case 'like':chartKeyword($scope.dataStatistic.keywordLikeCount.buckets);
                    break;
                case 'share':chartKeyword($scope.dataStatistic.keywordShareCount.buckets);
                    break;
            }
        }
